<!DOCTYPE html>
<html>
    <head>
        <meta charset="utf-8">
        <link rel="stylesheet" href="/src/css/fonts.css">
        <link rel="stylesheet" href="/src/css/position.css">
        <link rel="stylesheet" href="/src/css/style.css">
        <link rel="stylesheet" href="/src/css/rand/position.css">
        <link rel="stylesheet" href="/src/css/rand/style.css">
        <meta name="viewport" content="width=device-width, initial-scale=1" />
    </head>
    <body>
        <section class="words"></section>
        <main class="tools">
            <div class="tools__div tools-communication">
                <button class="tools-communication__button tools__button_chats">
                    <svg viewBox="0 0 32 32">
                        <path d="M25 20.822c0.414-0 0.75-0.336 0.75-0.75v0-18.072c-0-0.414-0.336-0.75-0.75-0.75h-23c-0.414 0-0.75 0.336-0.75 0.75v0 18.072c0 0.414 0.336 0.75 0.75 0.75h5.821v4.178c0 0.292 0.167 0.545 0.411 0.668l0.004 0.002c0.096 0.050 0.21 0.080 0.331 0.080 0.001 0 0.002 0 0.004 0h-0c0.17-0 0.326-0.057 0.452-0.152l-0.002 0.001 6.371-4.777zM14.692 19.473l-5.371 4.027v-3.428c-0-0.414-0.336-0.75-0.75-0.75h-5.821v-16.572h21.5v16.572h-9.107c-0.17 0-0.326 0.057-0.452 0.152l0.002-0.001zM30 9.25h-1c-0.414 0-0.75 0.336-0.75 0.75s0.336 0.75 0.75 0.75v0h0.25v14.215h-4.965c-0.414 0-0.75 0.336-0.75 0.75v0 2.785l-4.514-3.385c-0.123-0.094-0.279-0.15-0.449-0.15-0 0-0 0-0.001 0h-4.572c-0.414 0-0.75 0.336-0.75 0.75s0.336 0.75 0.75 0.75v0h4.322l5.514 4.135c0.123 0.094 0.28 0.15 0.449 0.15 0.123 0 0.238-0.030 0.34-0.082l-0.004 0.002c0.247-0.126 0.414-0.378 0.414-0.67v0-3.535h4.965c0.414-0 0.75-0.336 0.75-0.75v0-15.715c-0-0.414-0.336-0.75-0.75-0.75v0z"></path>
                    </svg>
                    <p class="tools-communication__p">4</p>
                </button>
                <div class="tools-communication__div tools-communication-timer">
                    <button class=" tools-communication-timer__button tools__button_more tools__button_pause">
                        <svg viewBox="0 0 24 24">
                            <path class="svg" fill-rule="evenodd" clip-rule="evenodd" d="M10.5 6H6V18H10.5V6ZM7.5 16.5V7.5H9V16.5H7.5ZM18 6H13.5V18H18V6ZM15 16.5V7.5H16.5V16.5H15Z"/>
                        </svg>
                    </button>
                    <p class="tools-communication-timer__p">
                        <span class="tools-communication-timer_hours">01</span>:<span class="tools-communication-timer_minutes">27</span>:<span class="tools-communication-timer_seconds">35</span>
                    </p>
                    <button class="tools-communication-timer__button tools__button_start tools__button_stop">
                        <svg viewBox="0 0 24 24">
                            <path class="svg" fill-rule="evenodd" clip-rule="evenodd" d="M6 6H18V18H6V6ZM16 16V8H8V16H16Z"/>
                        </svg>
                    </button>
                </div>
                <button class="tools-communication__button tools__button_notifications">
                    <svg viewBox="0 0 36 36">
                        <path d="M32.51,27.83A14.4,14.4,0,0,1,30,24.9a12.63,12.63,0,0,1-1.35-4.81V15.15A10.81,10.81,0,0,0,19.21,4.4V3.11a1.33,1.33,0,1,0-2.67,0V4.42A10.81,10.81,0,0,0,7.21,15.15v4.94A12.63,12.63,0,0,1,5.86,24.9a14.4,14.4,0,0,1-2.47,2.93,1,1,0,0,0-.34.75v1.36a1,1,0,0,0,1,1h27.8a1,1,0,0,0,1-1V28.58A1,1,0,0,0,32.51,27.83ZM5.13,28.94a16.17,16.17,0,0,0,2.44-3,14.24,14.24,0,0,0,1.65-5.85V15.15a8.74,8.74,0,1,1,17.47,0v4.94a14.24,14.24,0,0,0,1.65,5.85,16.17,16.17,0,0,0,2.44,3Z"></path>
                        <path  d="M18,34.28A2.67,2.67,0,0,0,20.58,32H15.32A2.67,2.67,0,0,0,18,34.28Z"></path>
                        <rect x="0" y="0" width="36" height="36" fill-opacity="0"/>
                    </svg>
                    <p class="tools-communication__p">5</p>
                </button>
            </div>
            <div class="tools__div tools-common">
                <div class="tools-common-actions">
                    <div class="tools-common__div tools-common__div_back">
                        <button class="tools-common__button tools-common__button_moreOftenUsed tools__button_splitList">
                            <svg class="tools-common__svg" viewBox="0 0 17 17">
                                <path d="M10.646 13.146l0.707 0.707-2.853 2.854-2.854-2.854 0.707-0.707 1.647 1.647v-3.772h1v3.772l1.646-1.647zM8 2.207v3.772h1v-3.772l1.646 1.646 0.707-0.707-2.853-2.853-2.854 2.853 0.707 0.707 1.647-1.646zM0 8v1h17v-1h-17z"/>
                            </svg>
                            <pre style="display:none;" class="tools-common__pre">Разделить список</pre>
                        </button>
                        <button class="tools-common__button tools-common__button_moreOftenUsed tools__button_combineLists">
                            <svg class="tools-common__svg" viewBox="0 0 16 16">
                                <path fill-rule="evenodd" clip-rule="evenodd" d="M1.5 1l-.5.5v3l.5.5h3l.5-.5v-3L4.5 1h-3zM2 4V2h2v2H2zm-.5 2l-.5.5v3l.5.5h3l.5-.5v-3L4.5 6h-3zM2 9V7h2v2H2zm-1 2.5l.5-.5h3l.5.5v3l-.5.5h-3l-.5-.5v-3zm1 .5v2h2v-2H2zm10.5-7l-.5.5v6l.5.5h3l.5-.5v-6l-.5-.5h-3zM15 8h-2V6h2v2zm0 3h-2V9h2v2zM9.1 8H6v1h3.1l-1 1 .7.6 1.8-1.8v-.7L8.8 6.3l-.7.7 1 1z"/>
                            </svg>
                            <pre style="display:none;" class="tools-common__pre">Объединить списки</pre>
                        </button>
                        <button class="tools-common__button tools-common__button_moreOftenUsed tools__button_createList">
                            <svg class="tools-common__svg" viewBox="0 0 32 32">
                                <path d="M24,15v2h-7v7h-2v-7H8v-2h7V8h2v7H24z M24.485,24.485c-4.686,4.686-12.284,4.686-16.971,0
                                    c-4.686-4.686-4.686-12.284,0-16.971c4.687-4.686,12.284-4.686,16.971,0C29.172,12.201,29.172,19.799,24.485,24.485z M23.071,8.929
                                    c-3.842-3.842-10.167-3.975-14.142,0c-3.899,3.899-3.899,10.243,0,14.142c3.975,3.975,10.301,3.841,14.142,0
                                    C26.97,19.172,26.97,12.828,23.071,8.929z"/>
                            </svg>
                            <pre style="display:none;" class="tools-common__pre">Создать список</pre>
                        </button>
                        <button class="tools-common__button tools-common__button_moreOftenUsed tools__button_next switcher">
                            <svg class="tools-common__svg" viewBox="0 0 1024 1024">
                                <path d="M256 120.768L306.432 64 768 512l-461.568 448L256 903.232 659.072 512z"/>
                            </svg>
                        </button>
                    </div>
                    <div style="display:none;" class="tools-common__div tools-common__div_next">
                        <button style="display:none;" class="tools-common__button tools__button_back switcher">
                            <svg class="tools-common__svg" viewBox="0 0 1024 1024">
                                <path d="M768 903.232l-50.432 56.768L256 512l461.568-448 50.432 56.768L364.928 512z"/>
                            </svg>
                        </button>
                        <button style="display:none;" class="tools-common__button tools__button_deleteList">
                            <svg class="tools-common__svg" viewBox="0 0 1024 1024">
                                <path d="M160 256H96a32 32 0 0 1 0-64h256V95.936a32 32 0 0 1 32-32h256a32 32 0 0 1 32 32V192h256a32 32 0 1 1 0 64h-64v672a32 32 0 0 1-32 32H192a32 32 0 0 1-32-32V256zm448-64v-64H416v64h192zM224 896h576V256H224v640zm192-128a32 32 0 0 1-32-32V416a32 32 0 0 1 64 0v320a32 32 0 0 1-32 32zm192 0a32 32 0 0 1-32-32V416a32 32 0 0 1 64 0v320a32 32 0 0 1-32 32z"/>
                            </svg>
                            <pre style="display:none;" class="tools-common__pre">Удалить список</pre>
                        </button>
                        <button style="display:none;" class="tools-common__button tools__button_shareList">
                            <svg class="tools-common__svg" viewBox="0 0 32 32">
                                <path d="M27 22c-1.646 0-3.103 0.8-4.013 2.028l-13.168-6.71c0.114-0.421 0.181-0.86 0.181-1.317 0-0.572-0.101-1.119-0.277-1.63l13.242-6.426c0.909 1.244 2.375 2.056 4.035 2.056 2.762 0 5-2.239 5-5s-2.238-5-5-5-5 2.239-5 5c0 0.388 0.049 0.764 0.133 1.127l-13.432 6.518c-0.915-1.009-2.231-1.646-3.7-1.646-2.761 0-5 2.239-5 5s2.239 5 5 5c1.59 0 3.004-0.744 3.92-1.902l13.222 6.739c-0.090 0.374-0.142 0.762-0.142 1.163 0 2.761 2.238 5 5 5s5-2.239 5-5-2.238-5-5-5zM27 2c1.657 0 3 1.343 3 3s-1.343 3-3 3-3-1.343-3-3 1.343-3 3-3zM5 19c-1.657 0-3-1.343-3-3s1.343-3 3-3c1.657 0 3 1.344 3 3s-1.343 3-3 3zM27 30c-1.657 0-3-1.343-3-3s1.343-3 3-3 3 1.343 3 3-1.343 3-3 3z"></path>
                            </svg>
                            <pre style="display:none;" class="tools-common__pre">Поделиться списком</pre>
                        </button>
                        <button style="display:none;" class="tools-common__button tools__button_editList">
                            <svg class="tools-common__svg" viewBox="0 0 24 24">
                                <path d="M4 16L3.46967 15.4697C3.32902 15.6103 3.25 15.8011 3.25 16H4ZM17 3L17.5303 2.46967C17.2374 2.17678 16.7626 2.17678 16.4697 2.46967L17 3ZM21 7L21.5303 7.53033C21.8232 7.23744 21.8232 6.76256 21.5303 6.46967L21 7ZM8 20V20.75C8.19891 20.75 8.38968 20.671 8.53033 20.5303L8 20ZM4 20H3.25C3.25 20.4142 3.58579 20.75 4 20.75V20ZM12 19.25C11.5858 19.25 11.25 19.5858 11.25 20C11.25 20.4142 11.5858 20.75 12 20.75V19.25ZM20.5 20.75C20.9142 20.75 21.25 20.4142 21.25 20C21.25 19.5858 20.9142 19.25 20.5 19.25V20.75ZM4.53033 16.5303L17.5303 3.53033L16.4697 2.46967L3.46967 15.4697L4.53033 16.5303ZM16.4697 3.53033L20.4697 7.53033L21.5303 6.46967L17.5303 2.46967L16.4697 3.53033ZM20.4697 6.46967L7.46967 19.4697L8.53033 20.5303L21.5303 7.53033L20.4697 6.46967ZM8 19.25H4V20.75H8V19.25ZM4.75 20V16H3.25V20H4.75ZM13.4697 6.53033L17.4697 10.5303L18.5303 9.46967L14.5303 5.46967L13.4697 6.53033ZM12 20.75H20.5V19.25H12V20.75Z"/>
                            </svg>
                            <pre style="display:none;" class="tools-common__pre">Изменить список</pre>
                        </button>
                    </div>
                </div>
                <div class="tools-common-search">
                    <button class="tools-common__button tools__button_search">
                        <svg viewBox="0 0 24 24">
                            <path d="M10.77 18.3C9.2807 18.3 7.82485 17.8584 6.58655 17.031C5.34825 16.2036 4.38311 15.0275 3.81318 13.6516C3.24325 12.2757 3.09413 10.7616 3.38468 9.30096C3.67523 7.84029 4.39239 6.49857 5.44548 5.44548C6.49857 4.39239 7.84029 3.67523 9.30096 3.38468C10.7616 3.09413 12.2757 3.24325 13.6516 3.81318C15.0275 4.38311 16.2036 5.34825 17.031 6.58655C17.8584 7.82485 18.3 9.2807 18.3 10.77C18.3 11.7588 18.1052 12.738 17.7268 13.6516C17.3484 14.5652 16.7937 15.3953 16.0945 16.0945C15.3953 16.7937 14.5652 17.3484 13.6516 17.7268C12.738 18.1052 11.7588 18.3 10.77 18.3ZM10.77 4.74999C9.58331 4.74999 8.42327 5.10189 7.43657 5.76118C6.44988 6.42046 5.68084 7.35754 5.22672 8.45389C4.77259 9.55025 4.65377 10.7566 4.88528 11.9205C5.11679 13.0844 5.68824 14.1535 6.52735 14.9926C7.36647 15.8317 8.43556 16.4032 9.59945 16.6347C10.7633 16.8662 11.9697 16.7474 13.0661 16.2933C14.1624 15.8391 15.0995 15.0701 15.7588 14.0834C16.4181 13.0967 16.77 11.9367 16.77 10.75C16.77 9.15869 16.1379 7.63257 15.0126 6.50735C13.8874 5.38213 12.3613 4.74999 10.77 4.74999Z"/>
                            <path d="M20 20.75C19.9015 20.7504 19.8038 20.7312 19.7128 20.6934C19.6218 20.6557 19.5392 20.6001 19.47 20.53L15.34 16.4C15.2075 16.2578 15.1354 16.0697 15.1388 15.8754C15.1422 15.6811 15.221 15.4958 15.3584 15.3583C15.4958 15.2209 15.6812 15.1422 15.8755 15.1388C16.0698 15.1354 16.2578 15.2075 16.4 15.34L20.53 19.47C20.6704 19.6106 20.7493 19.8012 20.7493 20C20.7493 20.1987 20.6704 20.3893 20.53 20.53C20.4608 20.6001 20.3782 20.6557 20.2872 20.6934C20.1962 20.7312 20.0985 20.7504 20 20.75Z"/>
                        </svg>
                    </button>
                </div>
            </div>
            <div class="tools__div tools-listsControl">
                <div class="tools-listsControl__div tools-listsControl-group">
                    <button class="tools-listsControl-group__button tools__button_personalLists">
                        <svg class="tools-listsControl-group__svg" viewBox="0 0 32 32">
                            <path d="M15.992 2c3.396 0 6.998 2.86 6.998 4.995v4.997c0 1.924-0.8 5.604-2.945 7.293-0.547 0.43-0.831 1.115-0.749 1.807 0.082 0.692 0.518 1.291 1.151 1.582l8.703 4.127c0.068 0.031 0.834 0.16 0.834 1.23l0.001 1.952-27.984 0.002v-2.029c0-0.795 0.596-1.045 0.835-1.154l8.782-4.145c0.63-0.289 1.065-0.885 1.149-1.573s-0.193-1.37-0.733-1.803c-2.078-1.668-3.046-5.335-3.046-7.287v-4.997c0.001-2.089 3.638-4.995 7.004-4.995zM15.992-0c-4.416 0-9.004 3.686-9.004 6.996v4.997c0 2.184 0.997 6.601 3.793 8.847l-8.783 4.145s-1.998 0.89-1.998 1.999v3.001c0 1.105 0.895 1.999 1.998 1.999h27.986c1.105 0 1.999-0.895 1.999-1.999v-3.001c0-1.175-1.999-1.999-1.999-1.999l-8.703-4.127c2.77-2.18 3.708-6.464 3.708-8.865v-4.997c0-3.31-4.582-6.995-8.998-6.995v0z"></path>
                        </svg>
                        <pre style="display:none;" class="tools-listsControl-group__pre">Личные списки</pre>
                    </button>
                    <button class="tools-listsControl-group__button tools__button_friendLists">
                        <svg class="tools-listsControl-group__svg" viewBox="0 0 32 32">
                            <path d="M23.313 26.102l-6.296-3.488c2.34-1.841 2.976-5.459 2.976-7.488v-4.223c0-2.796-3.715-5.91-7.447-5.91-3.73 0-7.544 3.114-7.544 5.91v4.223c0 1.845 0.78 5.576 3.144 7.472l-6.458 3.503s-1.688 0.752-1.688 1.689v2.534c0 0.933 0.757 1.689 1.688 1.689h21.625c0.931 0 1.688-0.757 1.688-1.689v-2.534c0-0.994-1.689-1.689-1.689-1.689zM23.001 30.015h-21.001v-1.788c0.143-0.105 0.344-0.226 0.502-0.298 0.047-0.021 0.094-0.044 0.139-0.070l6.459-3.503c0.589-0.32 0.979-0.912 1.039-1.579s-0.219-1.32-0.741-1.739c-1.677-1.345-2.396-4.322-2.396-5.911v-4.223c0-1.437 2.708-3.91 5.544-3.91 2.889 0 5.447 2.44 5.447 3.91v4.223c0 1.566-0.486 4.557-2.212 5.915-0.528 0.416-0.813 1.070-0.757 1.739s0.446 1.267 1.035 1.589l6.296 3.488c0.055 0.030 0.126 0.063 0.184 0.089 0.148 0.063 0.329 0.167 0.462 0.259v1.809zM30.312 21.123l-6.39-3.488c2.34-1.841 3.070-5.459 3.070-7.488v-4.223c0-2.796-3.808-5.941-7.54-5.941-2.425 0-4.904 1.319-6.347 3.007 0.823 0.051 1.73 0.052 2.514 0.302 1.054-0.821 2.386-1.308 3.833-1.308 2.889 0 5.54 2.47 5.54 3.941v4.223c0 1.566-0.58 4.557-2.305 5.915-0.529 0.416-0.813 1.070-0.757 1.739 0.056 0.67 0.445 1.267 1.035 1.589l6.39 3.488c0.055 0.030 0.126 0.063 0.184 0.089 0.148 0.063 0.329 0.167 0.462 0.259v1.779h-4.037c0.61 0.46 0.794 1.118 1.031 2h3.319c0.931 0 1.688-0.757 1.688-1.689v-2.503c-0.001-0.995-1.689-1.691-1.689-1.691z"></path>
                        </svg>
                        <pre style="display:none;" class="tools-listsControl-group__pre">Списки друзей</pre>
                    </button>
                    <button class="tools-listsControl-group__button tools__button_groupLists">
                        <svg class="svg-stroke tools-listsControl-group__svg" viewBox="0 0 32 32">
                            <style>.cls-1{fill:none;stroke-linecap:round;stroke-linejoin:round;stroke-width:2px;}</style>
                            <circle class="cls-1" cx="16" cy="13" r="5"/>
                            <path class="cls-1" d="M23,28A7,7,0,0,0,9,28Z"/>
                            <path class="cls-1" d="M24,14a5,5,0,1,0-4-8"/>
                            <path class="cls-1" d="M25,24h6a7,7,0,0,0-7-7"/>
                            <path class="cls-1" d="M12,6a5,5,0,1,0-4,8"/>
                            <path class="cls-1" d="M8,17a7,7,0,0,0-7,7H7"/>
                        </svg>
                        <pre style="display:none;" class="tools-listsControl-group__pre">Групповые списки</pre>
                    </button>
                </div>
                <div class="tools-listsControl__div tools-listsControl-cath">
                    <button class="tools-listsControl-cath__button tools-listsControl-cath_leftButton">
                        <svg style="display:none;opacity:0;" class="tools-listsControl-cath__svg" viewBox="0 0 1024 1024">
                            <path d="M768 903.232l-50.432 56.768L256 512l461.568-448 50.432 56.768L364.928 512z"/>
                        </svg>
                        <pre class="tools-listsControl-cath__pre">Наименование группы</pre>
                    </button>
                    <button class="tools-listsControl-cath__button tools-listsControl-cath_rightButton switcher">
                        <pre style="display:none;opacity:0;" class="tools-listsControl-cath__pre">Наименование категории</pre>
                        <svg class="tools-listsControl-cath__svg" viewBox="0 0 1024 1024">
                            <path d="M256 120.768L306.432 64 768 512l-461.568 448L256 903.232 659.072 512z"/>
                        </svg>
                    </button>
                </div>
            </div>
            <div class="tools__div tools-actionsArea">
                <div class="tools-actionsArea-editing">
                    <form class="tools-actionsArea-editing__form" action="#">
                        <input class="tools-actionsArea-editing__input" name="groupName" type="text" value="Наименование группы">
                        <input type="hidden" name="color">
                    </form>
                    <button class="tools-actionsArea-editing__button tools__button_selectCathColor"></button>
                </div>
                <div class="tools-actionsArea-confirmation">
                    <button class="tools-actionsArea-confirmation__button tools__button_addNewCath">
                        <svg class="tools-actionsArea-confirmation__svg" viewBox="0 0 24 24">
                            <path d="M13 3C13 2.44772 12.5523 2 12 2C11.4477 2 11 2.44772 11 3V11H3C2.44772 11 2 11.4477 2 12C2 12.5523 2.44772 13 3 13H11V21C11 21.5523 11.4477 22 12 22C12.5523 22 13 21.5523 13 21V13H21C21.5523 13 22 12.5523 22 12C22 11.4477 21.5523 11 21 11H13V3Z"/>
                        </svg>
                    </button>
                    <button class="tools-actionsArea-confirmation__button tools__button_createNesting">
                        <svg class="tools-actionsArea-confirmation__svg" viewBox="0 0 16 16" >
                            <path fill-rule="evenodd" d="M11 4a4 4 0 1 0 0 8 4 4 0 0 0 0-8zM6.025 7.5a5 5 0 1 1 0 1H4A1.5 1.5 0 0 1 2.5 10h-1A1.5 1.5 0 0 1 0 8.5v-1A1.5 1.5 0 0 1 1.5 6h1A1.5 1.5 0 0 1 4 7.5h2.025zM11 5a.5.5 0 0 1 .5.5v2h2a.5.5 0 0 1 0 1h-2v2a.5.5 0 0 1-1 0v-2h-2a.5.5 0 0 1 0-1h2v-2A.5.5 0 0 1 11 5zM1.5 7a.5.5 0 0 0-.5.5v1a.5.5 0 0 0 .5.5h1a.5.5 0 0 0 .5-.5v-1a.5.5 0 0 0-.5-.5h-1z"/>
                        </svg>
                    </button>
                    <button class="tools-actionsArea-confirmation__button tools__button_saveCathChanges">
                        <svg class="tools-actionsArea-confirmation__svg" viewBox="0 0 24 24">
                            <path d="M16.765 2c1.187 0 1.363.06 1.51.168L21.662 4.7a.845.845 0 0 1 .339.677v15.78a.844.844 0 0 1-.844.844H2.844A.844.844 0 0 1 2 21.156V2.844A.844.844 0 0 1 2.844 2zM17 21v-7H7v7zM14 3v3h1V3zM7 3v6h10V3h-1v4h-3V3zM3 21h3v-8h12v8h3V5.452l-3-2.278v6.17a.769.769 0 0 1-.844.656H6.844A.769.769 0 0 1 6 9.344V3H3z"/>
                        </svg>
                    </button>
                    <button class="tools-actionsArea-confirmation__button tools__button_deleteCath">
                        <svg class="tools-actionsArea-confirmation__svg" viewBox="0 0 1024 1024">
                            <path d="M160 256H96a32 32 0 0 1 0-64h256V95.936a32 32 0 0 1 32-32h256a32 32 0 0 1 32 32V192h256a32 32 0 1 1 0 64h-64v672a32 32 0 0 1-32 32H192a32 32 0 0 1-32-32V256zm448-64v-64H416v64h192zM224 896h576V256H224v640zm192-128a32 32 0 0 1-32-32V416a32 32 0 0 1 64 0v320a32 32 0 0 1-32 32zm192 0a32 32 0 0 1-32-32V416a32 32 0 0 1 64 0v320a32 32 0 0 1-32 32z"/>
                        </svg>
                    </button>
                </div>
            </div>
            <div class="tools__div tools-groupSelectionArea">
                <div class="tools-groupSelectionArea-navigationBlock">
                    <button class="tools-groupSelectionArea-navigationBlock__button tools-groupSelectionArea-navigationBlock_goBack">
                        <svg class="tools-groupSelectionArea-navigationBlock__svg" viewBox="0 0 1024 1024">
                            <path d="M768 903.232l-50.432 56.768L256 512l461.568-448 50.432 56.768L364.928 512z"/>
                        </svg>
                    </button>
                    <button class="tools-groupSelectionArea-navigationBlock__button tools-groupSelectionArea-navigationBlock_goHome">
                        <svg class="tools-groupSelectionArea-navigationBlock__svg" viewBox="0 0 36 36">
                            <path d="M33.71,17.29l-15-15a1,1,0,0,0-1.41,0l-15,15a1,1,0,0,0,1.41,1.41L18,4.41,32.29,18.71a1,1,0,0,0,1.41-1.41Z"/>
                            <path d="M28,32h-5V22H13V32H8V18L6,20V32a2,2,0,0,0,2,2h7V24h6V34h7a2,2,0,0,0,2-2V19.76l-2-2Z"/>
                        </svg>
                    </button>
                </div>
                <div class="tools-groupSelectionArea-itemsBlock">
                    <div class="tools-groupSelectionArea-cathegoryItem">
                        <button class="tools-groupSelectionArea-cathegoryItem__button tools__button_drag">
                            <svg class="tools-groupSelectionArea-cathegoryItem__svg" viewBox="0 0 24 24" fill="none">
                                <circle cx="20" cy="8" r="1" transform="rotate(-180 20 8)" stroke-width="2"/>
                                <circle cx="20" cy="16" r="1" transform="rotate(-180 20 16)" stroke-width="2"/>
                                <circle cx="12" cy="8" r="1" transform="rotate(-180 12 8)" stroke-width="2"/>
                                <circle cx="12" cy="16" r="1" transform="rotate(-180 12 16)" stroke-width="2"/>
                                <circle cx="4" cy="8" r="1" transform="rotate(-180 4 8)" stroke-width="2"/>
                                <circle cx="4" cy="16" r="1" transform="rotate(-180 4 16)" stroke-width="2"/>
                            </svg>
                        </button>
                        <pre class="tools-groupSelectionArea-cathegoryItem__pre">Наименование группы</pre>
                        <p class="tools-groupSelectionArea-cathegoryItem__p">8</p>
                    </div>
                    <div class="tools-groupSelectionArea-cathegoryItem">
                        <button class="tools-groupSelectionArea-cathegoryItem__button tools__button_drag">
                            <svg class="tools-groupSelectionArea-cathegoryItem__svg" viewBox="0 0 24 24" fill="none">
                                <circle cx="20" cy="8" r="1" transform="rotate(-180 20 8)" stroke-width="2"/>
                                <circle cx="20" cy="16" r="1" transform="rotate(-180 20 16)" stroke-width="2"/>
                                <circle cx="12" cy="8" r="1" transform="rotate(-180 12 8)" stroke-width="2"/>
                                <circle cx="12" cy="16" r="1" transform="rotate(-180 12 16)" stroke-width="2"/>
                                <circle cx="4" cy="8" r="1" transform="rotate(-180 4 8)" stroke-width="2"/>
                                <circle cx="4" cy="16" r="1" transform="rotate(-180 4 16)" stroke-width="2"/>
                            </svg>
                        </button>
                        <pre class="tools-groupSelectionArea-cathegoryItem__pre">Наименование группы</pre>
                        <p class="tools-groupSelectionArea-cathegoryItem__p">8</p>
                    </div>
                    <div class="tools-groupSelectionArea-cathegoryItem">
                        <button class="tools-groupSelectionArea-cathegoryItem__button tools__button_drag">
                            <svg class="tools-groupSelectionArea-cathegoryItem__svg" viewBox="0 0 24 24" fill="none">
                                <circle cx="20" cy="8" r="1" transform="rotate(-180 20 8)" stroke-width="2"/>
                                <circle cx="20" cy="16" r="1" transform="rotate(-180 20 16)" stroke-width="2"/>
                                <circle cx="12" cy="8" r="1" transform="rotate(-180 12 8)" stroke-width="2"/>
                                <circle cx="12" cy="16" r="1" transform="rotate(-180 12 16)" stroke-width="2"/>
                                <circle cx="4" cy="8" r="1" transform="rotate(-180 4 8)" stroke-width="2"/>
                                <circle cx="4" cy="16" r="1" transform="rotate(-180 4 16)" stroke-width="2"/>
                            </svg>
                        </button>
                        <pre class="tools-groupSelectionArea-cathegoryItem__pre">Наименование группы</pre>
                        <p class="tools-groupSelectionArea-cathegoryItem__p">8</p>
                    </div>
                </div>
            </div>
            <div class="tools__div tools-lists"></div>
            <div class="tools__div tools-quickAccessBar">
                <div class="tools-quickAccessBar-languages">
                    <button class="tools-quickAccessBar-languages__button tools__button_selectLanguage">mm</button>
                    <button class="tools-quickAccessBar-languages__button tools__button_selectLanguage">jp</button>
                </div>
                <div class="tools-quickAccessBar-modes">
                    <button class="tools-quickAccessBar-modes__button tools__button_showSource">
                        <svg class="tools-quickAccessBar__svg" viewBox="0 0 64 64" enable-background="new 0 0 64 64">
                            <path d="M56,0H8C5.789,0,4,1.789,4,4v56c0,2.211,1.789,4,4,4h48c2.211,0,4-1.789,4-4V4C60,1.789,58.211,0,56,0z M58,60c0,1.104-0.896,2-2,2H8c-1.104,0-2-0.896-2-2V4c0-1.104,0.896-2,2-2h48c1.104,0,2,0.896,2,2V60z"></path>
                            <path d="M49,25H15c-0.553,0-1,0.447-1,1s0.447,1,1,1h34c0.553,0,1-0.447,1-1S49.553,25,49,25z"></path>
                            <path d="M49,19H15c-0.553,0-1,0.447-1,1s0.447,1,1,1h34c0.553,0,1-0.447,1-1S49.553,19,49,19z"></path>
                            <path d="M49,37H15c-0.553,0-1,0.447-1,1s0.447,1,1,1h34c0.553,0,1-0.447,1-1S49.553,37,49,37z"></path>
                            <path d="M49,43H15c-0.553,0-1,0.447-1,1s0.447,1,1,1h34c0.553,0,1-0.447,1-1S49.553,43,49,43z"></path>
                            <path d="M49,49H15c-0.553,0-1,0.447-1,1s0.447,1,1,1h34c0.553,0,1-0.447,1-1S49.553,49,49,49z"></path>
                            <path d="M49,31H15c-0.553,0-1,0.447-1,1s0.447,1,1,1h34c0.553,0,1-0.447,1-1S49.553,31,49,31z"></path>
                            <path d="M15,15h16c0.553,0,1-0.447,1-1s-0.447-1-1-1H15c-0.553,0-1,0.447-1,1S14.447,15,15,15z"></path>
                        </svg>
                    </button>
                    <button class="tools-quickAccessBar-modes__button tools__button_showTranscription">
                        <svg class="tools-quickAccessBar__svg svg-stroke" viewBox="0 0 48 48">
                            <defs><style>.a{fill:none;stroke-linecap:round;stroke-linejoin:round;}</style></defs>
                            <path class="a" d="M32.7554,34.8415H8.1671A2.7945,2.7945,0,0,1,5.3727,32.047V8.2944A2.7944,2.7944,0,0,1,8.1671,5.5H22.531Z"></path>
                            <path class="a" d="M25.2,13.1585H39.8329a2.7945,2.7945,0,0,1,2.7944,2.7945V39.7056A2.7944,2.7944,0,0,1,39.8329,42.5H25.469L22.8,34.8414"></path>
                            <line class="a" x1="32.7554" y1="34.8415" x2="25.469" y2="42.5"></line>
                            <path class="a" d="M16.0441,11.0706h0a3.96,3.96,0,0,1,3.96,3.96v4.8958a3.96,3.96,0,0,1-3.96,3.96h0a3.96,3.96,0,0,1-3.96-3.96h0V15.0307a3.96,3.96,0,0,1,3.96-3.96Z"></path>
                            <path class="a" d="M9.4018,21.1048a6.7645,6.7645,0,0,0,13.2847,0"></path>
                            <line class="a" x1="16.0441" y1="26.5891" x2="16.0441" y2="29.9251"></line>
                            <line class="a" x1="27.9687" y1="21.1048" x2="39.2192" y2="21.1048"></line>
                            <line class="a" x1="31.3835" y1="30.9044" x2="39.2192" y2="30.9044"></line>
                            <line class="a" x1="29.7307" y1="26.0046" x2="39.2192" y2="26.0046"></line>
                        </svg>
                    </button>
                    <button class="tools-quickAccessBar-modes__button tools__button_showTranslation">
                        <svg class="tools-quickAccessBar__svg" viewBox="0 0 64 64" enable-background="new 0 0 64 64">
                            <path d="M60,0H14c-2.211,0-4,1.789-4,4v27H4c-2.211,0-4,1.789-4,4v23c0,3.313,2.687,6,6,6h52c3.313,0,6-2.687,6-6V4 C64,1.789,62.211,0,60,0z M2,58V35c0-1.104,0.896-2,2-2h6v25c0,2.209-1.791,4-4,4S2,60.209,2,58z M58,62H10.463 C11.416,60.938,12,59.539,12,58V4c0-1.104,0.896-2,2-2h46c1.104,0,2,0.896,2,2v54C62,60.209,60.209,62,58,62z"></path>
                            <path d="M53,25H21c-0.553,0-1,0.447-1,1s0.447,1,1,1h32c0.553,0,1-0.447,1-1S53.553,25,53,25z"></path>
                            <path d="M53,19H21c-0.553,0-1,0.447-1,1s0.447,1,1,1h32c0.553,0,1-0.447,1-1S53.553,19,53,19z"></path>
                            <path d="M53,37H21c-0.553,0-1,0.447-1,1s0.447,1,1,1h32c0.553,0,1-0.447,1-1S53.553,37,53,37z"></path>
                            <path d="M53,43H21c-0.553,0-1,0.447-1,1s0.447,1,1,1h32c0.553,0,1-0.447,1-1S53.553,43,53,43z"></path>
                            <path d="M53,49H21c-0.553,0-1,0.447-1,1s0.447,1,1,1h32c0.553,0,1-0.447,1-1S53.553,49,53,49z"></path>
                            <path d="M53,31H21c-0.553,0-1,0.447-1,1s0.447,1,1,1h32c0.553,0,1-0.447,1-1S53.553,31,53,31z"></path>
                            <path d="M21,15h15c0.553,0,1-0.447,1-1s-0.447-1-1-1H21c-0.553,0-1,0.447-1,1S20.447,15,21,15z"></path>
                        </svg>
                    </button>
                </div>
                <div class="tools-quickAccessBar-settings">
                    <button class="tools-quickAccessBar-settings__button tools__button_settings">
                        <svg class="tools-quickAccessBar__svg" viewBox="0 0 512 512">
                            <path d="M336.896,269.874c-18.795,0-36.449,7.308-49.698,20.575c-13.276,13.267-20.593,30.912-20.593,49.698 c0,18.768,7.307,36.422,20.593,49.707c13.276,13.267,30.921,20.574,49.698,20.574c18.769,0,36.422-7.307,49.698-20.574 c13.276-13.286,20.584-30.939,20.584-49.707c0-18.778-7.307-36.43-20.574-49.689C373.336,277.182,355.682,269.874,336.896,269.874z M378.1,381.35c-11.011,11.003-25.636,17.061-41.204,17.061c-15.559,0-30.202-6.058-41.204-17.061 c-11.011-11.002-17.07-25.635-17.07-41.203c0-15.568,6.058-30.202,17.07-41.195c11.002-11.011,25.645-17.069,41.204-17.069 c15.568,0,30.193,6.058,41.204,17.069c11.002,10.993,17.06,25.627,17.06,41.195C395.161,355.715,389.102,370.348,378.1,381.35z"/>
                            <path d="M499.041,317.118l-37.455-5.428c-2.346-0.18-4.368-1.654-5.277-3.838l-12.134-29.294 c-0.908-2.175-0.531-4.656,1.006-6.436l22.4-30.103c3.569-4.18,3.335-10.392-0.556-14.274l-17.726-17.726 c-2.04-2.05-4.736-3.083-7.433-3.083c-2.427,0-4.863,0.836-6.832,2.526l-30.112,22.409c-1.141,0.98-2.588,1.492-4.044,1.492 c-0.809,0-1.618-0.162-2.382-0.476l-29.312-12.153c-2.166-0.89-3.65-2.94-3.838-5.268l-5.438-37.455 c-0.422-5.484-4.989-9.708-10.472-9.708h-25.078c-5.492,0-10.059,4.224-10.481,9.708l-5.447,37.455 c-0.179,2.328-1.654,4.378-3.829,5.276l-29.312,12.144c-0.773,0.314-1.582,0.476-2.39,0.476c-1.456,0-2.895-0.512-4.045-1.492 l-30.094-22.4c-1.969-1.69-4.404-2.535-6.822-2.535c-2.706,0-5.402,1.043-7.452,3.092l-17.726,17.717 c-3.883,3.892-4.116,10.094-0.557,14.274l22.4,30.103c1.519,1.779,1.914,4.26,1.016,6.436l-12.144,29.303 c-0.881,2.175-2.931,3.65-5.258,3.829l-37.474,5.438c-5.464,0.432-9.689,4.998-9.689,10.49v25.06 c0,5.502,4.225,10.068,9.689,10.498l37.474,5.43c2.328,0.189,4.378,1.672,5.258,3.838l12.144,29.311 c0.907,2.158,0.503,4.656-1.016,6.436l-22.4,30.103c-3.559,4.161-3.326,10.382,0.557,14.265l17.726,17.726 c2.05,2.049,4.746,3.082,7.443,3.082c2.427,0,4.863-0.836,6.832-2.526l30.094-22.409c1.15-0.98,2.589-1.483,4.045-1.483 c0.809,0,1.617,0.152,2.39,0.476L304.6,459.57c2.175,0.899,3.659,2.93,3.838,5.276l5.438,37.456 c0.422,5.464,4.989,9.698,10.481,9.698h25.078c5.492,0,10.049-4.234,10.481-9.698l5.429-37.456 c0.189-2.345,1.672-4.376,3.838-5.276l29.312-12.144c0.773-0.324,1.581-0.476,2.39-0.476c1.456,0,2.904,0.503,4.045,1.483 l30.103,22.409c1.969,1.69,4.404,2.526,6.832,2.526c2.697,0,5.393-1.034,7.433-3.082l17.726-17.726 c3.9-3.883,4.125-10.104,0.556-14.265l-22.4-30.103c-1.509-1.78-1.914-4.279-1.006-6.436l12.134-29.311 c0.909-2.167,2.931-3.65,5.277-3.838l37.455-5.43c5.474-0.431,9.699-4.997,9.699-10.48v-25.078 C508.74,322.125,504.515,317.559,499.041,317.118z M496.722,351.364l-36.574,5.304c-6.643,0.71-12.324,4.944-14.939,11.182 l-12.126,29.266c-2.598,6.22-1.582,13.25,2.633,18.481l21.879,29.402l-15.856,15.855l-29.392-21.878 c-3.236-2.616-7.29-4.054-11.461-4.054c-2.436,0-4.808,0.485-6.984,1.402l-29.339,12.152c-6.211,2.589-10.444,8.278-11.155,14.93 l-5.302,36.574h-22.418l-5.312-36.565c-0.701-6.652-4.926-12.35-11.173-14.957l-29.276-12.117c-2.238-0.944-4.602-1.42-7.028-1.42 c-4.153,0-8.198,1.438-11.443,4.044l-29.411,21.887l-15.847-15.855l21.87-29.402c4.224-5.232,5.24-12.27,2.652-18.446 l-12.117-29.248c-2.553-6.256-8.252-10.517-14.94-11.236l-36.592-5.304v-22.436l36.575-5.303 c6.714-0.701,12.422-4.98,14.929-11.164l12.126-29.267c2.606-6.22,1.591-13.258-2.633-18.48l-21.879-29.411l15.865-15.856 l29.383,21.878c3.245,2.624,7.29,4.054,11.461,4.054c2.409,0,4.763-0.468,6.992-1.393l29.303-12.144 c6.23-2.571,10.481-8.278,11.173-14.948l5.321-36.566h22.418l5.302,36.584c0.72,6.669,4.972,12.369,11.173,14.921l29.312,12.153 c2.22,0.916,4.575,1.393,6.984,1.393c4.18,0,8.234-1.438,11.47-4.054l29.392-21.878l15.856,15.856l-21.86,29.375 c-4.243,5.214-5.268,12.26-2.661,18.471l12.135,29.312c2.588,6.22,8.288,10.462,14.948,11.164l36.565,5.303V351.364z"/>
                            <path d="M170.661,260.418l18.463-6.742c4.054-1.474,6.274-5.816,5.123-9.96l-6.066-29.05 c-0.486-1.798,0.062-3.686,1.411-4.926l18.328-16.826c0.907-0.836,2.094-1.276,3.29-1.276c0.584,0,1.169,0.099,1.716,0.296 l28.198,8.423c0.962,0.359,1.95,0.53,2.921,0.521c3.11-0.009,6.068-1.798,7.452-4.764l8.278-17.824 c1.843-3.91,0.332-8.557-3.416-10.67l-24.584-16.144c-1.6-0.907-2.562-2.624-2.472-4.476l1.061-24.853 c0.081-1.843,1.187-3.461,2.841-4.233l26.138-14.076c3.928-1.78,5.807-6.292,4.332-10.328l-6.732-18.471 c-1.214-3.336-4.378-5.43-7.776-5.42c-0.728,0.009-1.465,0.098-2.193,0.306l-29.052,6.049c-0.432,0.126-0.872,0.189-1.303,0.189 c-1.348,0.009-2.661-0.566-3.614-1.59l-16.808-18.32c-1.259-1.356-1.646-3.29-0.99-5.006l8.414-28.207 c1.501-4.035-0.332-8.53-4.252-10.355l-17.824-8.296c-1.133-0.522-2.329-0.773-3.506-0.773c-2.885,0.017-5.654,1.546-7.154,4.206 l-16.153,24.593c-0.872,1.528-2.49,2.471-4.243,2.48h-0.215l-24.863-1.078c-1.834-0.09-3.488-1.195-4.242-2.858L107.103,4.836 c-1.375-3.038-4.387-4.853-7.542-4.836c-0.926,0-1.869,0.162-2.786,0.494l-18.463,6.75c-4.054,1.475-6.283,5.807-5.114,9.951 l6.058,29.06c0.494,1.771-0.054,3.667-1.411,4.908L59.517,67.99c-0.925,0.845-2.094,1.286-3.299,1.295 c-0.575,0-1.159-0.108-1.726-0.324l-28.188-8.395c-0.961-0.369-1.95-0.539-2.921-0.539c-3.101,0.017-6.05,1.798-7.442,4.764 L7.662,82.624c-1.834,3.901-0.333,8.539,3.416,10.669l24.584,16.153c1.609,0.899,2.562,2.616,2.471,4.441l-1.06,24.871 c-0.072,1.834-1.196,3.461-2.85,4.234L8.085,157.068c-3.919,1.779-5.798,6.283-4.324,10.319l6.733,18.471 c1.222,3.335,4.386,5.447,7.784,5.438c0.719-0.009,1.447-0.117,2.175-0.323l29.069-6.058c0.423-0.117,0.854-0.18,1.277-0.18 c1.357,0,2.678,0.558,3.622,1.582l16.818,18.337c1.258,1.348,1.626,3.29,0.989,5.016l-8.414,28.188 c-1.5,4.036,0.342,8.539,4.252,10.374l17.815,8.286c1.141,0.522,2.337,0.773,3.514,0.773c2.886-0.018,5.654-1.537,7.146-4.197 l16.153-24.584c0.862-1.528,2.463-2.482,4.197-2.482h0.27l24.854,1.07c1.842,0.072,3.487,1.177,4.252,2.849l14.067,26.139 c1.384,3.02,4.386,4.836,7.55,4.827C168.801,260.903,169.744,260.75,170.661,260.418z M156.98,224.491 c-2.741-5.564-8.242-9.151-14.454-9.394l-24.763-1.06c-0.306-0.018-0.611-0.036-0.917-0.036 c-5.834,0.036-11.299,3.128-14.338,8.108L87.984,244.23l-11.883-5.528l7.569-25.321c1.986-5.887,0.628-12.314-3.569-16.845 l-16.79-18.309c-3.21-3.524-7.784-5.528-12.558-5.502c-1.33,0-2.661,0.162-3.946,0.486l-26.148,5.447l-4.477-12.297l23.514-12.656 c5.564-2.777,9.151-8.288,9.376-14.444l1.07-24.799c0.296-6.22-2.805-12.009-8.117-15.227L19.931,84.727l5.519-11.883l25.357,7.568 c1.762,0.594,3.595,0.881,5.465,0.881c4.206-0.018,8.225-1.581,11.371-4.458l18.274-16.773c4.629-4.206,6.535-10.498,5.034-16.539 l-5.438-26.121l12.296-4.494l12.646,23.513c2.751,5.547,8.234,9.133,14.446,9.393l24.889,1.079l0.746,0.018 c5.879-0.027,11.353-3.137,14.391-8.126l14.526-22.121l11.883,5.527l-7.56,25.304c-2.013,5.896-0.664,12.314,3.56,16.862 l16.817,18.328c3.208,3.514,7.775,5.51,12.512,5.483c1.349-0.009,2.679-0.17,3.991-0.476l26.121-5.456l4.476,12.305l-23.524,12.674 c-5.491,2.733-9.069,8.18-9.375,14.418l-1.07,24.781c-0.297,6.23,2.805,12.018,8.126,15.254l22.085,14.516l-5.519,11.892 l-25.348-7.577c-1.762-0.594-3.595-0.89-5.474-0.881c-4.234,0.008-8.279,1.609-11.362,4.44l-18.346,16.836 c-4.557,4.206-6.436,10.481-4.97,16.485l5.456,26.138l-12.306,4.477L156.98,224.491z"/>
                            <path d="M186.04,154.802c6.499-13.987,7.173-29.663,1.888-44.135c-5.294-14.48-15.891-26.04-29.842-32.521 c-7.792-3.64-16.062-5.465-24.575-5.429c-6.651,0.027-13.231,1.213-19.559,3.524c-14.498,5.285-26.057,15.883-32.556,29.86 c-6.499,13.96-7.173,29.626-1.897,44.134c5.304,14.49,15.911,26.049,29.861,32.53c7.784,3.622,16.053,5.455,24.565,5.42 c6.661-0.036,13.241-1.222,19.578-3.524C167.965,179.377,179.515,168.77,186.04,154.802z M149.376,173.372 c-5.042,1.843-10.282,2.769-15.505,2.787c-6.634,0.036-13.25-1.403-19.442-4.288c-11.065-5.151-19.442-14.302-23.64-25.762 c-4.17-11.478-3.65-23.882,1.501-34.939c5.151-11.074,14.31-19.469,25.77-23.639c5.051-1.852,10.292-2.778,15.506-2.796 c6.634-0.028,13.24,1.411,19.433,4.297c11.056,5.15,19.451,14.301,23.64,25.761c4.18,11.47,3.648,23.865-1.493,34.939 C169.978,160.797,160.837,169.184,149.376,173.372z"/>
                        </svg>
                    </button>
                </div>
            </div>
        </main>
        <section class="settings"></section>

        <script type="module" src="/src/js/view/tools/common/tools_common__actions.js"></script>

        <script type="module" src="/src/js/view/tools/listsControl/tools_listsControl.js"></script>
        <script type="module" src="/src/js/view/tools/listsControl/tools_listsControl__group.js"></script>
        <script type="module" src="/src/js/view/tools/listsControl/tools_listsControl__cath.js"></script>

        <script type="module" src="/src/js/view/tools/groupSelectionArea/tools_groupSelectionArea.js"></script>
        <script type="module" src="/src/js/view/tools/groupSelectionArea/tools_groupSelectionArea_itemsBlock.js"></script>
    </body>
</html>